...
void MapBlock::setNodeParent(v3s16 p, MapNode & n) {
	if (isValidPosition(p) == false) {
		...
		data[p.Z * MAP_BLOCKSIZE * MAP_BLOCKSIZE + p.Y * MAP_BLOCKSIZE + p.X] =
				n;
	}
}

FastFace * MapBlock::makeFastFace(u8 material, light_t light, v3f p, v3f dir,
		v3f scale, v3f posRelative_f) {
 ...
 */
void MapBlock::updateFastFaceRow(v3s16 startpos, u16 length,
		v3s16 translate_dir, v3s16 face_dir, core::list<FastFace*> &dest) {
	 ...
	 */
	v3f translate_dir_f(translate_dir.X, translate_dir.Y, translate_dir.Z); // floating point conversion
	v3f face_dir_f(face_dir.X, face_dir.Y, face_dir.Z); // floating point conversion
	...
	for (u16 j = 0; j < length; j++) {
		...
		if (j != length - 1) {
			...
			if (material0_next == material0 && material1_next == material1
					&& fabs(light_next - light) / (light + 0.01)
							< max_light_diff) {
		...
		if (next_is_different) {
			 ...
			 */
			if (material0 != material1 && material0 != MATERIAL_IGNORE
					&& material1 != MATERIAL_IGNORE
					&& (material0 == MATERIAL_AIR || material1 == MATERIAL_AIR)) {
				// Floating point conversion of the position vector
				v3f pf(p.X, p.Y, p.Z);
				// Center point of face (kind of)
				v3f sp = pf
						- ((f32) continuous_materials_count / 2. - 0.5)
								* translate_dir_f;
				v3f scale(1, 1, 1);
				...
				if (material0 != MATERIAL_AIR) {
					f = makeFastFace(material0, light, sp, face_dir_f, scale,
							posRelative_f);
				}
				// If node at sp is air
				else {
					f = makeFastFace(material1, light, sp + face_dir_f,
							-1 * face_dir_f, scale, posRelative_f);
				}
				dest.push_back(f);
			}
...
void MapBlock::updateFastFaces() {
	...
	for (s16 y = -1; y < MAP_BLOCKSIZE; y++) {
		for (s16 z = 0; z < MAP_BLOCKSIZE; z++) {
			updateFastFaceRow(v3s16(0, y, z), MAP_BLOCKSIZE, v3s16(1, 0, 0),
					v3s16(0, 1, 0), *fastfaces_new);
		}
	}
	/*
	...
	for (s16 x = -1; x < MAP_BLOCKSIZE; x++) {
		for (s16 y = 0; y < MAP_BLOCKSIZE; y++) {
			updateFastFaceRow(v3s16(x, y, 0), MAP_BLOCKSIZE, v3s16(0, 0, 1),
					v3s16(1, 0, 0), *fastfaces_new);
		}
	}
	/*

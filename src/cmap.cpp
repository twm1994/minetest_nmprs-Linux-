...
// ----------TODO: copied from getSectorNoGenerate(v2s16 p2d), decide whether to modify this method or leave it as it is----------
MapSector * Map::getSectorFromFile(v2s16 p2d){
...
MapSector * Map::getSector(v2s16 p2d)
{
  ...
  // return getSectorNoGenerate(p2d);
    return getSectorFromFile(p2d);
}

// ----------copied from getBlockNoCreate(v3s16 p3d)----------
MapBlock * Map::getBlockFromFile(v3s16 p3d)
{
  v2s16 p2d(p3d.X, p3d.Z);
  // MapSector * sectorref = getSectorNoGenerate(p2d);
  MapSector * sectorref = getSectorFromFile(p2d);
  MapSector *sector = sectorref;

  // ----------TODO: change this for appropriate method in MapSector----------
  ...
  core::map<v3s16, MapBlock*>  & modified_blocks)
{
  ...
  for (u16 i = 0; i < 6; i++) {
    ...
    try {
      ...
      block = getBlockFromFile(blockpos);
    }
  ...
  core::map<v3s16, MapBlock*> & modified_blocks)
{
  ...
  */
  ...
  try {
    ...
    block = getBlockFromFile(blockpos_last);
  }
  catch (InvalidPositionException &e)
  {
    return;
...
void Map::nodeAddedUpdate(v3s16 p, f32 lightwas)
{
  ...
  */
  ...
  MapBlock * blockref = getBlockFromFile(blockpos);
      ...
      for (s16 x = box_blocks.MinEdge.X; x <= box_blocks.MaxEdge.X; x++)
      {
        ...
        */
        try {
          ...
          MapBlock * blockref = getBlockFromFile(v3s16(x, y, z));
...
MapSector * MasterMap::getSector(v2s16 p2d)
{
  ...
  try {
    ...
    MapSector * sref = getSectorFromFile(p2d);
    return sref;
  }
  catch (InvalidPositionException &e)
  {
  // TODO: won't generate blocks on the fly, need to ensure player stays within boundary
  }
}
...
MapSector * ClientMap::getSector(v2s16 p2d)
{
  ...
  try {
    ...
    MapSector *sector = getSectorFromFile(p2d);
    return sector;
  }
  catch (InvalidPositionException &e)
  {

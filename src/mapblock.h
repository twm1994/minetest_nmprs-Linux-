...
enum {
	FACE_BACK = 0, FACE_TOP, FACE_RIGHT, FACE_FRONT, FACE_BOTTOM, FACE_LEFT
};
...
public:
	...
	bool isValidPosition(v3s16 p) {
		return (p.X >= 0 && p.X < MAP_BLOCKSIZE && p.Y >= 0
				&& p.Y < MAP_BLOCKSIZE && p.Z >= 0 && p.Z < MAP_BLOCKSIZE);
	}
	/*
	...
	MapNode * getNodePtr(s16 x, s16 y, s16 z) {
		if (x < 0 || x >= MAP_BLOCKSIZE)
			throw InvalidPositionException();
		if (y < 0 || y >= MAP_BLOCKSIZE)
			throw InvalidPositionException();
		if (z < 0 || z >= MAP_BLOCKSIZE)
			throw InvalidPositionException();
	...
	static FastFace * makeFastFace(u8 material, light_t light, v3f p, v3f dir,
	 ...
	 */
	void updateFastFaceRow(v3s16 startpos, u16 length, v3s16 translate_dir,
			v3s16 face_dir, core::list<FastFace*> &dest);
	/*
